...
class Storage(object):
  ...
  def __init__(self, hostName, port):
    self._db = Elasticsearch( [ { 'host': hostName, 'port':port } ] )
    self.__createIndex()


  def store(self, data):
    try:
      response = self._db.index( index=Storage._index_name, doc_type=Storage._doc_type, id=data["name"], body=data )
      #print( "data {result} in version {_version}".format(**response) )
      return response
    except Exception as e:
      print( "wrong input provided {}".format(data) )
